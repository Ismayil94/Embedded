...
// joystick vertical 󰀀 connected to J3.26 (analog)
...
// the loop inside the loop iterates 100 times and it has 1 ms delay in it, so Task1, Task3 and Task4 runs at about 10 Hz 

      for(int j=0; j<100; j++) { 

// there is 1 ms delay between each execution of Task0, so Task0 runs at about 1 kHz
      ...
      Task4();  // update the plot
		...
		// Because every iteration of infinite while loop takes about 100 ms * 10, so Task2 and Task5 both run at about 1 Hz
