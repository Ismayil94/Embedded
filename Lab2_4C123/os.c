...
int OS_AddPeriodicEventThreads(void(*thread1)(void), uint32_t period1,
  ...
  //***YOU IMPLEMENT THIS FUNCTION*****	
		Periodic_Thread1 = thread1;
		periodPt1 = period1; 
		Periodic_Thread2 = thread2;
		periodPt2 = period2;
    return 1;
}
...
void Scheduler(void){ // every time slice
  ...
  //***YOU IMPLEMENT THIS FUNCTION*****
	uint32_t LCM; // Least common multiple 
		LCM = periodPt1*periodPt2;// GIVE EVERYONE THE SAME AMOUNT OF TIME AND USE PREEMPTION TO SWITCH FROM ONE TASK TO ANOTHER
		Counter = (Counter + 1)% LCM; 
		if ((Counter % periodPt1)== 0){
			(*Periodic_Thread1)();
		}
		if ((Counter % periodPt2)== 1){
			(*Periodic_Thread2)();
		}
